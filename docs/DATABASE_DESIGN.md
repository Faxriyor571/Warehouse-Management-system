...
- **System Owner (Super Admin) scope — amended 2026-07-09:** the only role that legitimately operates *without* a single, standing company scope. Its own identity's data access is limited to the `companies` table itself (and `audit_logs` where `company_id IS NULL`) — it is never granted a blanket, always-on bypass on company-scoped business tables, unlike the legacy codebase's `is_superuser` flag, which bypasses every permission check unconditionally. Full operational access to a company's business data (originally out of scope for this role — see the superseded note this replaces) is instead granted through an explicit, time-boxed **support session** (`POST /companies/{id}/support-session`, API_SPECIFICATION.md §2): the request identity for that session resolves to a CEO-equivalent view scoped to one `company_id`, so the existing company-scoped query pattern is exercised unchanged — no table gains a Super-Admin bypass predicate. The session token carries the real System Owner's user id as its subject, so `audit_logs.user_id` still attributes every action to the real actor, never a fabricated one.
